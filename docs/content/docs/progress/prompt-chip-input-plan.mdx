...
1. **提示词面板** `desktop/renderer/src/components/canvas/canvas-node-prompt-panel.tsx` —— 本次要改的对象。`highlightLabels` 默认 true，value 是提示词字符串，引用以 label 文本「图片1」嵌在其中，**直接发给 AI**（配合 `desktop/renderer/src/lib/image-reference-prompt.ts` 的「参考图片编号：图片1」前缀）。
2. **节点内文本编辑** `desktop/renderer/src/components/canvas/canvas-node.tsx`（`isEditingContent` 分支）—— `highlightLabels={false}`，通过 `ref` 调用 `textarea.setSelectionRange(...)` 等 **textarea 专有 API**。若把组件改成 contentEditable，这处会直接崩。
...
新文件 `desktop/renderer/src/components/canvas/canvas-prompt-chip-input.tsx`，参照 `canvas-config-composer.tsx` 的 contentEditable 模式，但有一处**核心差异**：
...
- 新增：`desktop/renderer/src/components/canvas/canvas-prompt-chip-input.tsx`
- 修改：`desktop/renderer/src/components/canvas/canvas-node-prompt-panel.tsx`（仅替换输入框组件 + 相应 import）
