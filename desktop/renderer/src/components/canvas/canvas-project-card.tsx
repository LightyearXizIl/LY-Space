    ...
    return (
        <article className="group flex min-h-44 cursor-pointer flex-col justify-between rounded-2xl bg-[#f1eee8] p-5 transition hover:bg-[#ebe6dc] dark:bg-white/5 dark:hover:bg-white/10" onClick={() => !editing && open()}>
            <div className="flex items-start gap-3">
                ...
                />
                {editing ? (
                    <Input className="min-w-0 select-text" value={editingTitle} onClick={(event) => event.stopPropagation()} onChange={(event) => setEditingTitle(event.target.value)} onKeyDown={(event) => !isImeComposing(event) && event.key === "Enter" && saveTitle()} autoFocus />
                ) : (
                    <button
                        type="button"
