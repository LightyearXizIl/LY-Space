        ...
        <div ref={panelRef} style={style} onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
            ...
            {onCountChange ? (
                <div className="mt-4 space-y-2.5">
                    <div className="text-sm font-medium" style={{ color: theme.node.muted }}>生成数量</div>
                    <InputNumber
                        className="w-full"
                        min={1}
                        max={15}
                        precision={0}
                        value={count}
                        onChange={(value) => onCountChange(Math.max(1, Math.min(15, Math.floor(Math.abs(Number(value)) || 1))))}
                    />
                </div>
            ) : null}
        </div>,
