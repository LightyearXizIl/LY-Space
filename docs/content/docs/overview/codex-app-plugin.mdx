...
> 历史归档：桌面版不再打包或自动连接本地 Codex/Canvas Agent；本页仅说明仓库中保留的独立插件源码。
...
```text
请从 https://github.com/LightyearXizIl/LY-Space.git 安装 Infinite Canvas Codex 插件。
...
```bash
mkdir -p ~/plugins
git clone https://github.com/LightyearXizIl/LY-Space.git ~/plugins/infinite-canvas
```
...
插件会启动本地 Canvas Agent。桌面版会自动连接内置 Agent；需要在 Codex 浏览器中调试时，可启动本地渲染层并使用 Vite 地址连接画布。
